...
PREDICTION_QUEUE = 'queue_predictions'

# --- Task for Scheduled Runs ---
@app.task(bind=True, queue=PREDICTION_QUEUE)
def run_scheduled_flight_prediction(self, *args, **kwargs):
    """
    ...
    """
    logger.info(f"Running SCHEDULED flight prediction. Task ID: {self.request.id}")
...
@app.task(bind=True, queue=PREDICTION_QUEUE)
def handle_manual_prediction_request(self, raw_data_item):
    """
    Task executed when data is pushed to the 'raw_list_flight_prediction'
    and picked up by the watcher.
    """
    logger.info(f"Handling MANUAL flight prediction request. Task ID: {self.request.id}")
    ...
    try:
        ...
        logger.info("Manual flight prediction request finished successfully.")
        return result
    except Exception as e:
        logger.error(f"Error during manual flight prediction request: {e}", exc_info=True)
