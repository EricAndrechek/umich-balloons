...
int LoRaClass::begin(long frequency, Adafruit_MCP23X08* mymcp)
{
  setMcp(mymcp);
  // setup pins
  //pinMode(_ss, OUTPUT);
  _mcp->pinMode(LORA_CS_PIN, OUTPUT);
  // set SS high
  //digitalWrite(_ss, HIGH);
  _mcp->digitalWrite(LORA_CS_PIN, HIGH);

  if (_reset != -1) {
//    pinMode(_reset, OUTPUT);
    _mcp->pinMode(LORA_RESET_PIN, OUTPUT);

    // perform reset
    //digitalWrite(_reset, LOW);
    _mcp->digitalWrite(LORA_RESET_PIN, LOW);
    delay(10);
    //digitalWrite(_reset, HIGH);
    _mcp->digitalWrite(LORA_RESET_PIN, HIGH);
    delay(10);
  }

  // start SPI
  _spi->begin(LORA_CLK_PIN, LORA_MISO_PIN, LORA_MOSI_PIN);
...
void LoRaClass::setMcp(Adafruit_MCP23X08* mymcp)
{
  _mcp = mymcp;
}
...
uint8_t LoRaClass::singleTransfer(uint8_t address, uint8_t value)
{
  ...
  _mcp->digitalWrite(LORA_CS_PIN, LOW);
